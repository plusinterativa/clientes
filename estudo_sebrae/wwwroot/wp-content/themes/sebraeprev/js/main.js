(function($) {
	...
	//$('.fraseq').insertAfter('#tabs-1 .tab-hold.tabs-wrapper #tab1 .title h1');
})(jQuery);
...
jQuery(document).ready(function($) {
	...
	// check for specific tab id
	var href = document.URL;
	var tabid = href.substr(href.lastIndexOf('/') + 1);
	if(tabid.charAt(0) == '#') {
	var targetTab = jQuery(this).find("ul.tabs a[href=\"" + tabid + "\"]");
	if (targetTab.length) {
	targetTab.parent().addClass("active").show(); //Activate specific tab
	jQuery(this).find(tabid + ".tab_content ").show(); //Show specific tab content
	} else {
